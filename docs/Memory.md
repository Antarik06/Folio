...
- **Primary Tech Stack**: Next.js 16 (App Router), React 19, TypeScript, Express.js Backend, raw SQL via `node-postgres`, Supabase PostgreSQL, Konva, Three.js, Sharp, PDF-lib, Razorpay.
...
2. **Backend API Architecture**: Express server running in `backend/src/`, querying Supabase PostgreSQL with parameterised raw SQL through the `query()` wrapper in `src/db/index.ts`.
...
- ✅ Implemented core database schemas (Users, Events, Albums, Pages, Photos) as numbered `.sql` migrations.
