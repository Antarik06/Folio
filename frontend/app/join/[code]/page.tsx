  ...
  if (user) {
    if (event.host_id === user.id) {
      ...
      try {
        ...
        if (roleInfo) {
          guestStatus.isGuest = roleInfo.isGuest
          guestStatus.isCollaborator = roleInfo.isCollaborator
          guestStatus.faceEnrolled = roleInfo.faceEnrolled ?? false
        }
      } catch (err) {
        console.error('Error fetching guest role on join landing:', err)
      }
    }
  }

  const host = { full_name: event.host_name || '' }
