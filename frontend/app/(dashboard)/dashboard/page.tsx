...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ mode?: string }> }) {
  ...
  const { events = [], guestEntries = [], albums = [], orders = [] } = dashboardData

  // Map to match JSX expectations
  ...
  const mappedGuestEntries = guestEntries.map((entry: any) => ({
    id: entry.id,
    role: entry.role,
    ...
    events: {
      ...
      cover_image_url: entry.cover_image_url || entry.latest_photo_url || null,
      status: entry.status,
      profiles: {
        full_name: entry.host_name
      }
    }
  }))
  ...
  const mappedOrders = orders.map((order: any) => ({
    ...order,
    albums: {
      title: order.album_title
    }
  }))

  const stats = {
    events: mappedEvents.length,
    guestEvents: mappedGuestEntries.length,
    albums: mappedAlbums.length,
    pendingOrders: mappedOrders.filter((o: any) => o.status !== 'delivered').length,
  }
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="mb-16 md:mb-20">
        <Link
          ...
          className="group relative flex flex-col md:flex-row items-center justify-between p-8 bg-foreground text-background dark:bg-card dark:text-foreground border border-border overflow-hidden"
        >
          {/* Skew overlay */}
          <div className="absolute top-0 right-0 w-1/2 h-full bg-primary/20 dark:bg-primary/10 -skew-x-12 transform translate-x-1/2 transition-transform group-hover:translate-x-1/3 duration-700" />

          {/* Content */}
          <div className="relative z-10 space-y-4 text-center md:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-background/10 border border-border rounded-full text-[10px] font-mono uppercase tracking-[0.2em]">
            ...
            <p className="text-muted-foreground/90 max-w-md text-sm leading-relaxed">
          ...
          <div className="mt-8 md:mt-0 relative z-10 flex items-center gap-4">
            ...
            <div className="hidden sm:flex -space-x-3 mr-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="w-12 h-14 bg-white shadow-lg flex items-center justify-center p-1 pb-4 transform rotate-3 even:-rotate-6 first:rotate-12 transition-all duration-500 group-hover:rotate-0 group-hover:-translate-y-1"
                >
                  <div className="w-full h-full bg-gray-200 dark:bg-gray-200" />
                </div>
              ))}
            </div>

            {/* CTA icon */}
            <div className="bg-primary text-white h-12 w-12 flex items-center justify-center rounded-md group-hover:scale-110 transition-transform">
      ...
      {guestEntries && guestEntries.length > 0 && (
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {guestEntries.map((entry: any) => {
              const event = entry.events as any
              if (!event) return null
              const eventCover = event.cover_image_url
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <span className={`text-xs uppercase tracking-wider px-2 py-1 ${albumStatus === 'ready'
