...
export function Sidebar({
  ...
  const runAiAssistant = React.useCallback(async (task: 'analyze' | 'caption') => {
    ...
    try {
      ...
      const data = await apiClient.post('/api/ai/assistant', {
        task: 'analyze-image',
        image: base64,
        prompt
      })
  ...
  const loadRemoteElements = React.useCallback(
    async (page: number, reset: boolean) => {
      ...
      try {
        ...
        const data = await apiClient.get(`/api/elements/search?${params.toString()}`)
  ...
  React.useEffect(() => {
    async function fetchAlbums() {
      ...
      try {
        ...
        console.error('Failed to fetch albums in editor sidebar:', err)
      } finally {
        setLoading(false)
      }
    }
