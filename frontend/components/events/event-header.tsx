...
export function EventHeader({ event, isHost, photoCount, guestCount }: EventHeaderProps) {
  return (
    <div className="mb-8">
      {/* Breadcrumb */}
      <div className="mb-6">
        <Link href="/photos/events" className="text-sm text-muted-foreground hover:text-foreground transition-colors">
          ← Back to Events
        </Link>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="font-serif text-3xl md:text-4xl text-foreground">{event.title}</h1>
        ...
        {isHost && (
          <div className="flex items-center gap-3">
            <Link
              href={`/photos/events/${event.id}/settings`}
